...
import pprint
import operator
from ngram.LetterModels import Unigram, Bigram
...
class TamilVUNgram:
    def __init__(self):
        ...
        print(u"--- completed Bigram model ---")
    ...
    def save(self):
        ...
        with codecs.open("tvu_bigram.txt","w","utf-8") as fp:
            ...
            #    for k2,v in sorted(v.items(),key=operator.itemgetter(1),reverse=True):
            #        if v == 0:
            #            continue #pass
            #        fp.write(u"%s,%s - %d\n"%(k,k2,v))
            # makeit a 1D dictionary
            d = {}
            for k,v in self.bigram.letter2.items():
